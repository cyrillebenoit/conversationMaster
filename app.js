...
if (cloudantUrl) {
  ...
  app.get('/chats', auth, function(req, res) {
    logs.list({
      ...
      // download as CSV
      var csv = [];
      csv.push(['Question', 'Intent', 'Confidence', 'Entity', 'Output',
        'Time'
      ]);
      ...
      body.rows.forEach(function(row) {
        ...
        if (row.doc) {
          ...
          if (doc.response) {
            intent = '<no intent>';
            if (doc.response.intents && doc.response.intents.length >
              0) {
        ...
        csv.push([question, intent, confidence, entity, outputText,
          time
        ]);
      });
      res.csv(csv);
